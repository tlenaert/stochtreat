...
void PatientData::tokenize(const string& str, vector<string>& tokens, const string& delimiters){
...
void PatientData::processTimepoints(vector<string>& tokens, int p){
	for (unsigned i=1; i<tokens.size()-1; ++i) {
		_timepoints.push_back(boost::lexical_cast<float>(tokens[i]));
	}
}

void PatientData::processPatientdata(vector<string>& tokens, int p){
	for (unsigned i=1; i<tokens.size()-1; ++i) {
		_data.push_back(boost::lexical_cast<float>(tokens[i]));
	}
}

void PatientData::processData(vector<string>& tokens, int p){
	vector<float> tmp;
	for (unsigned i=1; i<tokens.size()-1; ++i) {
...
void PatientData::parseBuffer(stringstream& ss){
	string line;
	...
	if(!patient_found && _patient >0){
		cout << "!! Requested patient " << _patient << " NOT FOUND !! " << endl;
		exit(-1);
	}
}


void PatientData::readBinaryFile(istream& is){
	is.seekg (0, ios::end);
...
void PatientData::transformAllData(vector< vector<float> >& transformed){
...
ostream& PatientData::display(ostream& os){
	os << "[" << _patient << ":";
...
//	cout << _data[_estimate.size()-1] << "\t" << _pool.diseaseBurden() << "\t" << _estimate.size() << endl;;
}
...
void PatientData::recordMonthly(Model& _pool){
	//store disease burden in _monthly vector
...
double PatientData::estimateError(){
	double err(0.0);
//	cout << _estimate.size() << " \t " <<  _timepoints.size() << endl;
...
//		cout << _data[i] << "\t" << _estimate[i] << "\t" << _tquartile[i] << "\t" << _fquartile[i] << endl;
//		cout << _estimate[i] << " \t";
		if(_data[i] != -1)
