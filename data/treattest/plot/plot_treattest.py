...
fig.subplots_adjust(left=0.18,bottom=0.21,right=0.96,top=0.96)
ax.margins(0.05)

filenames=sys.argv[1:]

cmap = plt.get_cmap('viridis')
indices = np.linspace(0, cmap.N, 2)
my_colors = itertools.cycle([cmap(int(i)) for i in indices])
labels=itertools.cycle(['All patients','No LSC at diag.'])
...
for filename in filenames:
    ...
    color=next(my_colors)
    label=next(labels)
    ax.plot(data[:,0]*lengthening,data[:,2],alpha=0.7,color=color,label=label )#,marker="o",markersize=2.0,linestyle="None")
...
ax.legend()
plt.savefig("/Users/boettcher/Documents/HierarchicalCellModel/manusscripts/drafts/figures/treattest.pdf")
# plt.show()
